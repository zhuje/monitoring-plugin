...
export const refreshSilences = (
  ...
  const url = getFetchSilenceAlertUrl(perspective, 'refreshSilences', activeNamespace);
  if (!url) {
    return;
  }
  ...
  consoleFetchJSON(
    getFetchSilenceAlertUrl(perspective, 'refreshSilences - consoleFetchJSON', activeNamespace),
  )
    .then((silences) => {
