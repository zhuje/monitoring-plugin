...
import { useFetchPersesDashboards } from './useFetchPersesDashboards';
...
const MonitoringDashboardsPage_: React.FC<MonitoringDashboardsPageProps> = ({ history, match }) => {
  ...
  const [persesBoards] = useFetchPersesDashboards();
  ...
  const boardItems = React.useMemo(() => {
    ...
    console.log({ persesBoards });

    if (persesBoards) {
      const persesKey = _.mapKeys(persesBoards, function (item) {
        return item?.metadata?.name;
      });
      const persesBoardItems = _.mapValues(persesKey, (b, name) => ({
        tags: ['perses'],
        title: `${b.metadata?.project} / ${b.metadata?.name}` ?? name,
      }));
      console.log('ALL TOGETHER : ', { ...persesBoardItems, ...ocpBoardItems });
